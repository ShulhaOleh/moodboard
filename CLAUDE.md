...
- **Dexie** — IndexedDB wrapper for local persistence. Board state is saved under the key `'default'` and loaded on startup; `scheduleSave()` debounces writes (1500 ms), `flushSave()` fires synchronously on `visibilitychange`/`pagehide`/`beforeunload`. Schema is versioned via `SCHEMA_VERSION` in `src/lib/db.ts` (currently 5). To add a new version: bump `SCHEMA_VERSION`, and if any block field changed in a breaking way, add an entry to the `MIGRATIONS` dict in `db.ts` keyed by the version it produces — `migrateBlocks()` runs all entries between the stored version and `SCHEMA_VERSION` automatically. Purely additive changes (new optional fields) require no migration entry. `MIN_SUPPORTED_VERSION` controls the oldest accepted save; only raise it to intentionally drop old data.
...
**Groups** (`main.ts`, `src/lib/db.ts`): Groups are modelled as a `Map<string, GroupRecord>` (`{ id, name }`) in `main.ts`, not as a container block. Each block carries `groupId?: string` (a mutable field on `BaseBlock`; pseudo-objects `CanvasBoard` and `PencilTool` expose `readonly groupId = undefined`). `groupCounter` is a monotonically increasing integer used to name new groups (`Group 1`, `Group 2`, …) — it never decrements on ungroup or undo, so numbers never repeat within a session; it is persisted in `BoardRecord.groupCounter` and restored on load so numbers don't reset across sessions either. `groupSelected()` (Ctrl+G) assigns a fresh UUID to all selected blocks; `ungroupSelected()` (Ctrl+Shift+G) clears `groupId` and calls `cleanupEmptyGroups()`. Click-to-select behavior in `main.ts`: a plain click on a grouped block selects all unlocked-visible members first; a second click (when the group is already fully selected) drills into the individual block. `paste()` remaps group IDs via a `groupRemap` map so pasted groups form new parallel groups. Groups are persisted in `BoardRecord.groups?: GroupRecord[]` (schema v5) and cleared alongside blocks on new board / import / load-demo.
...
**Layers panel** (`src/ui/LayersPanel.ts`): Left-docked panel listing all blocks in z-order (last `blocks[]` entry = frontmost = top row). Supports drag-to-reorder (HTML5 DnD), per-row visibility and lock toggles, inline name editing (double-click or rename shortcut), and multi-select via Ctrl+click (toggle) and Shift+click (contiguous range). Mirrors the dock/undock/collapse/resize behavior of `PropertiesPanel`. `onReorder` callback in `main.ts` splices `blocks[]` then calls `overlay.appendChild` on every block to re-sync DOM order. `refresh()` rebuilds all rows; `notifySelectionChanged()` only toggles `is-selected` classes. The panel uses `data-array-index` on each row to map between the reversed visual order and the actual array index. The rename key is injected via `layersPanel.isRenameKey` from `main.ts` so it respects the user's configured binding. Grouped blocks render as a collapsible tree: `buildDisplayItems()` collects same-`groupId` blocks into `GroupItem` containers positioned at the z-order of their frontmost member; each group shows a chevron header with its own vis/lock/rename controls, and member rows are indented with CSS tree connectors (├ / └ via `::before`/`::after` pseudo-elements on `.layer-member-row`). Multi-select uses an `anchorKey` (set on every plain or Ctrl click, not on Shift click) and `getFlatVisibleItems()` / `computeRange()` to determine the range; collapsed group members are excluded from the flat list. The panel fires `onSelectBlock` / `onGroupSelect` (plain), `onCtrlSelectBlock` / `onCtrlGroupSelect` (toggle), and `onRangeSelect` (range) — all wired in `main.ts`.

**Settings and keybindings** (`src/lib/settings.ts`, `src/lib/keybindings.ts`, `src/ui/SettingsPanel.ts`): User preferences live in `localStorage`. `UserSettings` (key `moodboard-settings`) holds `theme` (`'light' | 'dark' | 'system'`), `accent` (`AccentColor`), and `uiFont` (string). `applyTheme()` sets `data-theme` on `<html>`; `applyAccent()` sets `data-accent`; `applyUiFont()` sets `font-family` on `<html>`. Call `beginThemeTransition()` immediately before either `applyTheme` or `applyAccent` when the user triggers the change — it briefly adds a `theme-transition` class to `<html>` so CSS can animate color properties; do NOT call it on the initial startup apply (no animation on load). `KeybindingMap` (key `moodboard-keybindings`) maps each `ShortcutAction` to `ActionBindings = { primary: Keybinding; secondary: Keybinding | null }` — every action supports an optional second binding. `matchesAction(e, bindings)` checks both slots; `ctrl: true` accepts either Ctrl or Cmd. All board shortcut checks in `main.ts` use `matchesAction`; `LayersPanel.isRenameKey` is the one exception wired via a callback. `SettingsPanel` is a full-screen overlay with a Discord-style sidebar — `open()` registers a capture-phase Escape listener that intercepts before the board's handler. To add a new shortcut: add the action to `ShortcutAction` and `DEFAULT_KEYBINDINGS` in `keybindings.ts`; add a `TranslationKey` entry to `ACTION_TRANSLATION_KEYS` in `SettingsPanel.ts`; add it to the `actions` array in `SettingsPanel.buildKeybindingsPage()`; use `matchesAction(e, keybindings.yourAction)` in `main.ts`. If the shortcut should be reflected in a UI hint (e.g. the mode dropdown), update `syncModeHints()` in `main.ts` and `AddBar.updateModeHints()` accordingly — `syncModeHints` is called on load and inside `onKeybindingsChange` so hints stay in sync when the user rebinds.

**Translations** (`src/translations.ts`, locale files in `src/translations/`): Only ever edit `src/translations/en.json` — all other locale files are maintained by human translators and must not be modified by AI. All user-visible strings go through `t(key)` — never hardcode UI text. `TranslationKey` is derived from `en.json` via `keyof typeof en`, so typos are TypeScript errors. The fallback chain is `current[key] ?? en[key] ?? key`. `loadSavedLocale()` reads the chosen locale from `localStorage` (`moodboard-locale`) and must be called once at startup (after `applyUiFont`). New locales are added by dropping a `<code>.json` file in `src/translations/` with a `"_name"` key; Vite's `import.meta.glob` picks them up automatically. Each UI component that has static text registers `onLocaleChange(fn)` in its constructor to update stored element refs when the locale changes — panels with dynamic content (e.g. `PropertiesPanel`) re-call `show(object)` in their handler to fully rebuild. Do not bake keyboard shortcut strings into translation values — toolbar button hints are assembled at runtime via `AddBar.updateButtonHints()`, called from `syncModeHints()` in `main.ts` which has access to the live keybinding map. Keybinding row labels in `SettingsPanel` use `ACTION_TRANSLATION_KEYS: Record<ShortcutAction, TranslationKey>` (defined in `SettingsPanel.ts`) to look up the translated label at render time and on every `onLocaleChange` callback.
...
src/
  ...
  translations.ts     # t(), onLocaleChange(), setLocale(), loadSavedLocale(), getLocales(), getCurrentLocale()
  translations/
    en.json           # canonical locale — TranslationKey is derived from this file's keys
  lib/
